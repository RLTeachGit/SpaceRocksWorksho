...
public class FakePhysics : MonoBehaviour {
	...
	void Start () {
        mHeight = Camera.main.orthographicSize;   //This is slow as it seaches scene for camera         mWidth = mHeight * Camera.main.aspect;
    }
...
         if (transform.position.y > mHeight) {             transform.position -= Vector3.up * 2 * mHeight;   //Offset to other side of screen
        } else if (transform.position.y < -mHeight) {             transform.position -= Vector3.down * 2 * mHeight;         }
         if (transform.position.x > mWidth) {             transform.position -= Vector3.right * 2 * mWidth;         } else if (transform.position.x < -mWidth) {             transform.position -= Vector3.left * 2 * mWidth;         }
    }
